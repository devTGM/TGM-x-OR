if (!customElements.get('recipient-form')) {
  customElements.define(
    ...
    class RecipientForm extends HTMLElement {
      ...
      connectedCallback() {
        // Only the product form carries productVariantId. Every other cartUpdate
        // publisher — cart-items, cart-dedupe, gift-wrapping, quick-order-list —
        // omits it, and `undefined.toString()` threw on each one. The pubsub
        // wrapper swallows the throw, so it never surfaced as anything worse than
        // console noise, but the handler bailed before doing its job.
        this.cartUpdateUnsubscriber = theme.pubsub.subscribe(theme.pubsub.PUB_SUB_EVENTS.cartUpdate, (event) => {
          if (event.source !== 'product-form' || event.productVariantId == null) return;
          if (event.productVariantId.toString() === this.currentProductVariantId) {
        ...
        // Was assigned over cartUpdateUnsubscriber, so the cartUpdate subscription
        // could never be torn down and leaked for the life of the page while this
        // one was never removed at all.
        this.cartErrorUnsubscriber = theme.pubsub.subscribe(theme.pubsub.PUB_SUB_EVENTS.cartError, (event) => {
          if (event.source !== 'product-form' || event.productVariantId == null) return;
          if (event.productVariantId.toString() === this.currentProductVariantId) {
